...
- [X] T030 Constitution-principle attestation captured in `specs/003-multi-network-protocol/constitution-attestation.md`. Per-principle: **I** Backward-compat — N=1 single-network path preserved, optional fields, FR-005 only on v4 branch; **II** E2E per transport — 4 scaffolds (P2P, WC2, popup, FR-019), runtime gated on real network; **III** Real-network — scaffolds + reference wallets hit ghostnet (L1) + previewnet (L2); **IV** Reference parity — both ref dApps + both ref wallets demonstrate the documented pattern, T029 grep clean; **V** Spec & guide as deliverables — integration guide §3a + §4 matrix + website + README all updated. Open compliance gap: runtime green-light on full matrix (T023) + SDK unit tests (T008/T012, optional).
- [X] T031 Bucket A SDK PR finalized. Spec 002 + spec 003 SDK commits ride on `trilitech/octez.connect#31` ("Multi-network support for Tezos X", DRAFT, head: `feat/peer-version-handshake`). Bucket A landed as commit `4aa67868` ("Add multi network support and version based diffrentiation for dapp and wallet requests") on top of the spec 002 commits (`6b44bc60` → `82b6094b`). PR #31 description amended to cover both specs with full backward-compatibility matrix + validation checklist + cross-link to outer-repo PR #4. No separate upstream PR opened (per spec Clarifications Session 2026-05-20).
- [X] T032 Bucket B outer-repo PR opened: `trilitech/tezos-x-octez-connect#4` "feat: multi-network protocol ergonomics (spec 003)", OPEN, base: `002-peer-version-handshake` (stacks behind spec 002 PR #3). Body covers spec 003 scope, ref-app refactor, integration guide §3a + §4 updates, e2e scaffolds, grep verification baselines (6→0 / 4→0 / 1→0 / 2→0), and a test plan referencing the Bucket A SDK commit id `4aa67868` from `trilitech/octez.connect#31`. Five subsequent commits land scaffold pairing fixes + the rate-limit sleep that makes `test:mn-p2p` GREEN end-to-end against real ghostnet + Tezos X previewnet (SC-007).
...
- **C1 (HIGH)** — ~~T016 + T017 now spell out the wallet emit-side FR-005 rejection: the dispatch table's default branch MUST emit a wire-level structured error for unsupported chain ids before building any partial `accounts[]`.~~ **REVISED 2026-06-01:** FR-005 is now partial fulfillment — the wallet builds `accounts[]` for the served subset and omits unsupported chains (optionally advertising them via advisory `unsupportedNetworks`). There is no wallet-side wire-level rejection for unsupported networks; the dApp decides. See spec.md Clarifications Session 2026-06-01.
