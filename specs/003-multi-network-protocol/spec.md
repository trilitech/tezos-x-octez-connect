...
- Q: The dApp SDK code committed as `trilitech/octez.connect@4aa67868` branches on the presence of `blockchainData.accounts` (in `DAppClient.requestPermissions` and `TezosBlockchain.getAccountInfosFromPermissionResponse`) to choose between the v4 multi-network and v3 legacy paths. This contradicts spec 002 / constitution C5 ("`peer.version` is the sole routing key — no field-presence detection of v4-era fields"). Should the dApp SDK be fixed to route on `peer.version` only, symmetric with FR-014 on the wallet side? → A: A — Yes. Switch both decision sites to branch on `Number(walletPeerVersion) >= 4` (sourced from `PeerManager.getPeer()`, same as `assertWalletVersionMeetsMinimum`). The `accounts[]` map is **data** consumed inside the v4 branch only, never a routing signal. The blockchain method's signature gains a `peerVersion` parameter so it can make the decision without re-deriving from the message envelope. FR-019 defensive still fires when `peer.version >= '4'` AND `accounts[]` is absent; otherwise the v3 path is used regardless of what's in `blockchainData`. New FR-021 added below to codify the rule. Code fix tracked as a follow-up commit on the SDK PR (`trilitech/octez.connect#31`).

### Session 2026-06-01

- Q: When a wallet cannot serve every requested network, who decides whether the resulting session is acceptable — the wallet (wire-level whole-request rejection, the Session 2026-05-20 answer) or the dApp? → A: **The dApp.** This reverses the Session 2026-05-20 Q1 answer, on review feedback (`tezos-x-octez-connect#4`). The wallet returns the subset of accounts it *can* serve and omits the rest (partial fulfillment); the dApp inspects what it got (`getAccounts()`, per Q3 / FR-003) and decides whether that subset is enough for its flow. Rationale: (1) the wallet doesn't know the dApp's intent, so it shouldn't arbitrate which networks are *mandatory* — that's dApp policy; (2) the "ambiguous half-fulfilled session" that justified wallet-side rejection was already eliminated by Q3 (all N served accounts are persisted and individually visible), so the dApp now sees exactly what it got; (3) graceful degradation: a Tezos-only wallet can still connect a dual-runtime bridge dApp for its L1 flow instead of blocking the user entirely. The wallet MAY include an OPTIONAL advisory `unsupportedNetworks: string[]` field in the response (the information, without the decision) so the dApp can distinguish "wallet doesn't know this chain" from "user has no account here" for precise messaging. `NetworksUnsupportedBeaconError` stays exactly as built (client-side only); it is now raised by the dApp SDK when the served subset is **empty** (F1) or when a v4 wallet returns **no `accounts[]` fanout at all** (F4 / FR-019 — a genuine capability gap, distinct from a partial-but-non-empty subset). FR-005 rewritten; edge case 1 + Key Design Decision "Whole-request rejection" + conformance rule C6 (integration guide §4) updated accordingly.
...
- **Wallet doesn't support one of the requested networks.** Default behavior (per Clarifications Session 2026-06-01): partial fulfillment with a dApp-side decision. The wallet returns `accounts[]` for the subset it *can* serve, omits the unsupported chains, and MAY add an optional advisory `unsupportedNetworks` field. The dApp inspects `getAccounts()` and decides whether the subset is enough for its flow (e.g. a bridge dApp can proceed with L1-only). If the wallet can serve *none* of the requested networks (empty subset), the dApp SDK raises `NetworksUnsupportedBeaconError`.
...
- **An older v4 wallet (built against spec 002 but not 003) receives a multi-network request.** Per spec 002 rules the wallet is at its own max for the wire-level `peer.version` exchange, but it cannot serve `networks[]`-fanout at all. Because it is unaware of multi-network semantics, it returns **no `accounts[]` map** (or only a legacy single-network shape). The dApp SDK detects this missing-fanout case via post-handshake validation and raises `NetworksUnsupportedBeaconError` (FR-019 / F4). Note this is distinct from a spec-003-aware wallet that *understands* fanout but serves only a subset — that is valid partial fulfillment (FR-005), not a capability gap, and does not raise.
...
- **FR-005**: When a dApp requests permissions and the wallet cannot serve all requested networks, the wallet MUST partially fulfill: it returns `accounts[]` populated with one entry per requested network it *can* serve and omits the networks it cannot. The wallet MUST NOT reject the whole request on the grounds that some networks are unsupported — whether a partial session is acceptable is the dApp's decision, not the wallet's (the wallet does not know which networks the dApp treats as mandatory). The wallet MAY include an OPTIONAL advisory `unsupportedNetworks: string[]` field in the response naming the omitted chain ids (informational only; it carries no decision). The dApp SDK persists every served account via `getAccounts()` (FR-003) and exposes the advisory field if present; the dApp then inspects the served set and decides whether to proceed (graceful degradation) or surface an error. If the served subset is empty (the wallet could serve *none* of the requested networks), the dApp SDK raises `NetworksUnsupportedBeaconError`. (Resolved by Clarifications Session 2026-06-01, reversing the Session 2026-05-20 whole-request-rejection answer.)
...
- **FR-019**: If `peer.version >= '4'` (routing key per FR-021) AND a v4 wallet response carries **no `accounts[]` fanout at all** for a `requestPermissions` call carrying `networks.length >= 2`, the dApp SDK MUST detect the shape mismatch and raise `NetworksUnsupportedBeaconError` with `requestedNetworks` + `unsupportedNetworks` populated. This is the genuine capability gap: a wallet that advertises v4 on the wire but does not understand spec-003 multi-network fanout. Silently treating such a wallet as multi-network-capable is FORBIDDEN. **This is distinct from FR-005 partial fulfillment**: a wallet that *does* understand fanout and returns a non-empty subset of `accounts[]` is partially fulfilling (FR-005), not failing the FR-019 gate — the SDK accepts the served subset and does not raise. FR-019 fires only on an absent/empty `accounts[]`. The four-condition gate (per [research.md](./research.md) R6) prevents false positives on single-network v4 sessions and on dApps that explicitly relaxed `requiredMinimumVersion` below `'4'`.
...
- **FR-021** *(symmetric with FR-014 on the wallet side)*: The dApp SDK MUST use the wallet's `peer.version` — sourced from `PeerManager.getPeer()`, the same authoritative store `assertWalletVersionMeetsMinimum` already reads — as the **sole routing key** for choosing between the v4 multi-network code path and the v3 legacy code path in `DAppClient.requestPermissions` AND in the `Blockchain.getAccountInfosFromPermissionResponse` interface implementations (`TezosBlockchain`, future blockchain modules). Field-presence detection of `blockchainData.accounts` (or any other v4-era field) as a routing signal is FORBIDDEN. The `accounts` map is **data** consumed inside the v4 branch only; if `peer.version >= '4'` but `accounts[]` is absent, FR-019 defensive applies. If `peer.version < '4'`, the legacy single-network path is used regardless of what `blockchainData` contains. The `Blockchain.getAccountInfosFromPermissionResponse` method signature MUST be extended to accept the resolved `peerVersion: string` so it can make the decision without re-deriving from the message envelope (the inner-envelope version is the hardcoded legacy compat stamp — not the routing key). Resolved by Clarifications Session 2026-05-22.
...
- **Partial fulfillment with a dApp-side decision (revised 2026-06-01).** If a wallet cannot serve every network in a `permission_request`, it returns the subset of `accounts[]` it can serve and omits the rest; it does NOT reject the whole request. Whether a partial session is acceptable is the dApp's decision — the wallet doesn't know the dApp's intent and so must not arbitrate which networks are mandatory. The wallet MAY attach an optional advisory `unsupportedNetworks` field (the information, without the decision). The earlier "whole-request rejection" design (mirroring spec 002's `VersionUnsupportedBeaconError`) was reversed because the half-fulfilled-session ambiguity it guarded against was already removed once we committed to persisting all N served accounts via `getAccounts()` (Clarifications Q3) — the dApp now sees exactly what it got. This enables graceful degradation (e.g. a bridge dApp connecting a Tezos-only wallet for its L1 flow). `NetworksUnsupportedBeaconError` (client-side) still fires when the served subset is empty (F1) or when a v4 wallet returns no fanout at all (F4 / FR-019).
...
- **`wallet/src/index.ts` test-config endpoint (spec 002 T046)**: The `POST /test-config` control surface that lets the test harness pin BEACON_VERSION on the wallet side. Reused here to scaffold the FR-019 defensive path (v4-multinet dApp vs. v4-but-singlenet wallet).
