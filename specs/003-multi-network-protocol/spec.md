...
- **FR-019**: If `peer.version >= '4'` (routing key per FR-021) AND a v4 wallet response is missing the `accounts[]` fanout for a `requestPermissions` call carrying `networks.length >= 2`, the dApp SDK MUST detect the shape mismatch and raise `NetworksUnsupportedBeaconError` with `requestedNetworks` + `unsupportedNetworks` populated. Silently treating an older v4 wallet as multi-network-capable is FORBIDDEN. The four-condition gate (per [research.md](./research.md) R6) prevents false positives on single-network v4 sessions and on dApps that explicitly relaxed `requiredMinimumVersion` below `'4'`.
